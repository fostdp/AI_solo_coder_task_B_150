...
import { applyDeadzone, applyResponseCurve, applySensitivity, processAxis, calibrateAxis } from './gamepadUtils'
import type {
  DrivingState,
  GamepadState,
  ResponseCurveType,
...
const keyMap: Record<string, KeyMapping> = {
...
export function VRWoodenOx() {
  ...
  useGamepad({ setGamepadState, calibrating, setCalibration })
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (calibration.calibrated) {
        axisX = calibrateAxis(axisX, calibration.centerX, calibration.maxX)
        axisY = calibrateAxis(axisY, calibration.centerY, calibration.maxY)
      }

      const steering = processAxis(axisX, deadzone, responseCurve, sensitivity)
      const acceleration = processAxis(-axisY, deadzone, responseCurve, sensitivity)
    ...
    return () => clearInterval(interval)
  }, [controlMode, gamepadState, sendControl, deadzone, responseCurve, sensitivity, getMappedButton, calibration])
            ...
            <div className="space-y-4">
              ...
              <Row gutter={16} className="mt-2">
                <Col span={12}>
                  ...
                  <Progress percent={Math.abs(processAxis(gamepadState.axisX, deadzone, responseCurve, sensitivity)) * 100} status="normal" showInfo={false} />
                  <div className="text-xs text-gray-400 mt-1">{processAxis(gamepadState.axisX, deadzone, responseCurve, sensitivity).toFixed(3)}</div>
                </Col>
                <Col span={12}>
                  <div className="mb-1 text-sm text-gray-500">加速输出 (处理后)</div>
                  <Progress percent={Math.abs(processAxis(-gamepadState.axisY, deadzone, responseCurve, sensitivity)) * 100} status="normal" showInfo={false} />
                  <div className="text-xs text-gray-400 mt-1">{processAxis(-gamepadState.axisY, deadzone, responseCurve, sensitivity).toFixed(3)}</div>
                </Col>
              </Row>
              <Row gutter={16} className="mt-2">
                <Col span={6}>
              ...
              <div className="space-y-2">
                <Row gutter={16}>
                  <Col span={12}>
                    ...
                    <div className="text-sm text-green-600">X轴 (处理后): {processAxis(gamepadState.axisX, deadzone, responseCurve, sensitivity).toFixed(3)}</div>
                  </Col>
                  <Col span={12}>
                    <div className="text-sm">Y轴 (原始): {gamepadState.axisY.toFixed(3)}</div>
                    <div className="text-sm text-green-600">Y轴 (处理后): {processAxis(gamepadState.axisY, deadzone, responseCurve, sensitivity).toFixed(3)}</div>
                  </Col>
                </Row>
                <Row gutter={16}>
...
export { VRWoodenOx as VirtualDriving }
