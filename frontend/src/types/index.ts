...
export interface TransportMetrics {
  ...
  stability_on_slope: number
  energy_efficiency: number
  payload_capacity: number
  speed_on_flat: number
  speed_on_slope: number
  terrain_adaptability: number
}

export interface TransportComparisonResult {
  wooden_ox: TransportMetrics
  wheelbarrow: TransportMetrics
  horse_carriage: TransportMetrics
...
export interface TransportRadarData {
  terrain_type: string
  axes: RadarAxis[]
  datasets: RadarDataset[]
  raw_metrics: Record<string, TransportMetrics>
}

export interface EraMetrics {
  mechanical_complexity: number
  max_obstacle_height: number
  max_slope_angle: number
  speed: number
  payload_ratio: number
  autonomy_hours: number
  terrain_types_supported: number
  noise_level_db: number
  cost_estimate_relative: number
  control_method: string
  power_source: string
  sensing_capability: number
...
export interface EraRadarAxis {
  axis: string
  value: number
  normalized: number
}

export interface EraTimelineEvent {
  year: number
  era: string
  event: string
  description: string
...
export interface CargoStabilityPoint {
  x: number
  z: number
  min_margin: number
  avg_margin: number
...
export interface CargoStabilityGridResult {
  grid: CargoStabilityPoint[]
  optimal_position: { x: number; z: number }
  dangerous_zones: Array<{ x: number; z: number; reason: string }>
  safe_zone_boundary: Array<{ x: number; z: number }>
}

export interface CargoHeightPoint {
  height: number
  min_margin: number
  avg_margin: number
  tipping_risk: number
...
export interface CargoMassPoint {
  mass: number
  min_margin: number
  avg_margin: number
  tipping_risk: number
...
export interface DrivingControlInput {
  device_id: string
  acceleration: number
  steering: number
  brake: number
